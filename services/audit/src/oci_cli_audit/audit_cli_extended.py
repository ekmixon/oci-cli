...
def list_events(ctx, from_json, all_pages, compartment_id, start_time, end_time, page, skip_deserialization, stream_output):
    ...
    if stream_output:
        ...
        if ctx.obj['debug']:
            ...
            end_render = timer()
            print(
                f"Time elapsed for total rendering response: {end_render - start_render}",
                file=sys.stderr,
            )
    ...
    if ctx.obj['debug']:
        end_command = timer()
        print(
            f"Time elapsed for total command execution: {end_command - start_command}",
            file=sys.stderr,
        )
